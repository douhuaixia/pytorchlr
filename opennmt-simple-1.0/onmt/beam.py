...
class Beam():
  ''' Beam Search '''
  def __init__(self, size, decode_length, alpha=0.6, stop_early=True, device=False):
    self.size = size
    self.alpha = alpha
    ...
    self.alive_seq[0][0] = Constants.BOS
    # alive_seq: (size, 1)
    self.alive_log_prob = torch.zeros((size,), dtype=torch.float, device=device)
    # alive_log_prob: (size, )
    
    # The score for each finished translation on the beam
    self.finished_seq = torch.zeros(self.alive_seq.size(), dtype=torch.long, device=device) + Constants.EOS
    # finished_seq: (size, 1)
  ...
  def grow_finished(self, curr_seq, curr_scores, curr_finished):
    ...
    finished_seq = torch.cat((self.finished_seq, torch.zeros((self.size, 1), dtype=torch.long, device=self.device) + Constants.EOS), dim=1)
    # finished_seq: (size, ? + 1)
  ...
  def advance(self, word_prob):
    ...
    topk_finished = topk_ids.eq(Constants.EOS)

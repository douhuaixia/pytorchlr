...
def get_sinusoid_encoding_table(n_position, d_hid, padding_idx=None):
    ...
    # 这看起来是53的原因，多出来的第0维全部置为0， 原因是在dataset.py中序列编号中出现了0<blank>，而这些0
    #　是没有位置意义的，所以直接置０?
    # 另外从dataset.py中可知位置信息的编号是从1开始的, 最后面应该全部为0
...
# 这里传入的参数相同，seq_k = seq_q = seq_src， 64*max_len == batch*m
def get_attn_key_pad_mask(seq_k, seq_q):
    ...
    # padding_mask_unsqueeze(1)==> size: 64*1*max_len
    # .expand(-1,len_q, -1)==> size: 64*max_len*max_len, max_len是复制
    #１那个维度的数值ｍax_len次所得

    # 尺寸为：(batch, m, m)
    # 效果是[0][0] = [0][1] = [0][2] = ... = [0][m-1]
...
# 入口：
# 原句中的字典    n_src_vocab=n_src_vocab       2911
# 每一句最长不超过  len_max_seq=len_max_seq       52
# 词向量大小    d_word_vec=d_word_vec         512
# encoder堆叠的层数 n_layers=n_layers             6
# muliti_attention的数量  n_head=n_head                 8
# q、k的维度   d_k=d_k                       64
# v的维度   d_v=d_v                       64
# 每个单词编码后的维度，与词向量的维度相同， d_model=d_model               512
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        # (2911, 512)
        # 总共出现的单词数量为n_src_vocab, 每个单词被编码为d_word_vec维，如果单词为<blank>
        # 则对应的d_word_vec为0, 之所以会出现<blank>是因为确保同一batch里面的所有句子长度相同而
        # 人为添加的
        self.src_word_emb = nn.Embedding(
            n_src_vocab, d_word_vec, padding_idx=Constants.PAD)

        #  nn.Embedding.from_pretained : 用第一个参数(Tensor)的维度与数值构造并初始化Embedding,
        # freeze = True表示不更新Embedding, 也就是位置信息是固定的
        # 53是因为每一句话中的单词编号范围为0-52
    ...
    def forward(self, src_seq, src_pos, return_attns=False):
        ...
        # -- Forward
        # src_seq : 64*max_len, src_pos : 64*max_len  64为batch_size
        # self.position_enc是一个定值，src_pos起的是索引的作用
        # enc_output: 64*max_len*512, 512为词向量的大小
...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, src_pos, tgt_seq, tgt_pos):
        ...
        # 丢弃最后一列，tgt_seq丢掉的东西是</s>, tgt_pos大小与tgt_seq大小保持一致
        # 为什么丢弃?
        tgt_seq, tgt_pos = tgt_seq[:, :-1], tgt_pos[:, :-1]
        # 参数src_seq为(64, max_len, 512)
        # enc_output.size() ==> (64, max_len, 512)
